...
public class Lec169_Majority_Element {
  ...
  public static int majorityElement(int[] nums) {
    /**
     * Approach - if there exists a majority element in the array there should be atleast 1 element
     * remaining if any 2 elements cancel out each other
     */
    ...
    for (int i = 1; i < nums.length; i++) {
      ...
      // if next element is different this cancels out 1 of its prev element (i.e decrement majority
      // count by 1 )
      if (majorityElementFoundSoFar != nums[i]) {
        majorityElementCount--;
      }

      // if majority element Count is < 0 that indicates till this point in the array elements
      // cancelled out each other and there is no majority element
