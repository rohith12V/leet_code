 ...
 *
 * <p>important points to take away : Tweaking BST to find left most value and right most matching
 * value case - 1 ( normal algorithm ) if key is found return the index
 *
 * <p>case 2 ( tweaked version ) to make bst get the left most index when we find the element at mid
 * then shift the right pointer to mid - 1; [ start -> mid -1 ] ( new search ) to make bst get the
 * right most index when we find the element at mid then shift the left pointer to mid + 1; [ mid+1
 * -> end ] ( new search ) *
 */
  ...
  public int searchRangeUsingBST(int[] nums, int target, boolean leftBias) {
    ...
    while (start <= end) {
      ...
      // if middle element is less than target i.e. our target lies in the second half
        ...
        if (leftBias) {
          // if its left biased we have to make sure end will always shift its position towards
          // start [ start , mid - 1 ]
          end = mid - 1;
        } else {
          // *** if its right biased we have to make sure start will always shift its position
          // towards end [ mid + 1 , end ]
          start = mid + 1;
        }
      }
    }
