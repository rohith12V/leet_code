...
public class Lec162_Peak_Element {
  ...
  public int findPeakElement(int[] nums) {
    ...
    while (low <= high) {
      ...
      // condition - 3  [1,2,3,6,4,3,2,1] mid == 3 arr[2] < arr[3] > arr[4] return index 3, which is
      // also mid
      ...
      // right or left depends upon the increasing side ( which has a probability of getting a local
      // max )
      if (mid + 1 < nums.length && nums[mid] < nums[mid + 1]) {
        // shift low towards mid + 1 as arr[mid+1] is > arr[mid] which has an increasing sequence,
        // and probably we find peak element
        low = mid + 1;
      } else {
        // shift high towards mid - 1 as arr[mid-1] is > arr[mid] which has an increasing sequence,
        // and probably we find peak element
        high = mid - 1;
      }
    }
